...
The [free symmetric group](https://doi.org/10.1016/j.jfa.2006.09.005) $S_d^+$ is the noncommutative analogue of the classical symmetric group. It is generated by the entries of a magic unitary matrix $P=(P_{ij})$, whose entries are orthogonal projections and whose rows and columns each sum to one. One can calculate averages over the Haar state of the free symmetric quantum group using `haar_integral_free_symmetric`.

Similarly, the [free orthogonal group](https://doi.org/10.2977/prims/1201011782) $O_d^+$ is the noncommutative analogue of the classical orthogonal group. It is generated by self-adjoint elements $O_{ij}$ satisfying the orthogonality relations $OO^T=I_d$, where the matrix entries need not commute. One can calculate averages over the Haar state of the free symmetric quantum group using `haar_integral_free_orthogonal`.
