...
Haarpy is a Python library for the symbolic calculation of [Weingarten functions](https://en.wikipedia.org/wiki/Weingarten_function) and related averages of matrix ensembles under their [Haar measure](https://pennylane.ai/qml/demos/tutorial_haar_measure): these include the [classical compact groups](https://arxiv.org/abs/math-ph/0609050) (orthogonal, unitary and symplectic) their associated [circular ensembles](https://en.wikipedia.org/wiki/Circular_ensemble), the permutation and centered permutation groups, and quantum groups. For more information, please see the [documentation](https://haarpy.readthedocs.io).
...
The [free symmetric group](https://doi.org/10.1016/j.jfa.2006.09.005) $S_d^+$ is the noncommutative analogue of the classical symmetric group. It is generated by the entries of a magic unitary matrix $P=(P_{ij})$, whose entries are orthogonal projections and whose rows and columns each sum to one. One can calculate averages over the Haar state of the free symmetric group using `haar_integral_free_symmetric`.

Similarly, the [free orthogonal group](https://doi.org/10.2977/prims/1201011782) $O_d^+$ is the noncommutative analogue of the classical orthogonal group. It is generated by the self-adjoint elements of a matrix $O=(O_{ij})$ satisfying the orthogonality relation $OO^T=I_d$, where the matrix entries need not commute. One can calculate averages over the Haar state of the free orthogonal group using `haar_integral_free_orthogonal`.
