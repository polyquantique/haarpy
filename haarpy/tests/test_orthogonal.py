...
def test_zonal_spherical_orthogonality_symmetric_none_zero(permutation, partition):
    ...
    double_partition = tuple(2 * i for i in partition)
    orthogonality = Fraction(
        factorial(degree), ap.irrep_dimension(double_partition)
    ) * ap.zonal_spherical_function(permutation, partition)
...
def test_weingarten_orthogonal_coset_type(degree):
    ...
    permutation_sample = (Permutation.random(degree) for _ in range(sample_size))
    for permutation in permutation_sample:
        assert ap.weingarten_orthogonal(permutation, d) == ap.weingarten_orthogonal(
            ap.coset_type(permutation), d
        )
...
def test_weingarten_orthogonal_class_dimension_type_error(coset_type, dimension):
    "Test type error for orthogonal Weingarten function"
    with pytest.raises(
        TypeError,
...
def test_haar_integral_orthogonal_gorin_collins_reconcile(monomial, structure):
    ...
    collins_symb = ap.haar_integral_orthogonal(monomial, d, "collins", structure)
...
def test_haar_integral_orthogonal_string_value_error(string_value, parameter):
...
def test_haar_integral_orthogonal_tuple_format_value_error(sequences):
...
def test_zero_column_integral_orthogonal(column):
    ...
    assert not _column_integral_orthogonal(column, d)
